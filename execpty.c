/*
 * execpty version 1.1 - Execute a command in pseudo terminal redirecting pty
 * I/O to standard I/O.
 *
...
// Number of milliseconds that stdin is not polled once EOF is detected
#define EOF_TIMEOUT  100

static char *myname;
static char *buffer;
static pid_t cpid = 0;
static int ret_value = E_ERROR;
static int quiet = 0;
static int noecho = 0;
static int cr_output = 0;
...
int main(int argc, char **argv) {
  ...
  if ((mfd = posix_openpt(O_RDWR | O_NOCTTY)) == -1 || grantpt(mfd)
      || unlockpt(mfd)) {
    error("posix_openpt(), grantpt() or unlockpt()");
  }
  ...
  while (1) {
    if (poll(fds, pollable_fds, poll_timeout) == -1) {
      error("poll()");
    }
    ...
    if (fds[0].revents & POLLIN) {
      data_transfer(mfd, STDOUT_FILENO);
    }
    ...
    /* We have to stay in the loop in case of STDIN either EOF or pipe write end
     * close to continue output transfer.
     * POLLIN @ EOF temporary disables poll on STDIN when POLLHUP disables it
     * permanently.
     */
    if (fds[1].revents & POLLIN) {
      data_transfer(STDIN_FILENO, mfd);
    }
    if (fds[1].revents & POLLHUP) {
      // Pipe write end is closed.
      // We will not poll the pipe any more.
      pollable_fds = max_pollable_fds = 1;
    }
  ...
  if (WIFSIGNALED(wstatus)) {
    ret_value = WTERMSIG(wstatus) + 128;
  }

  // Close pty only AFTER child exited, otherwise it is killed by SIGHUP
  close(mfd);
  // Prevent tio access after main's return
  terminal_restore();
  return ret_value;
}
...
static void data_transfer(int fromfd, int tofd) {
  ...
  do {
    if ((rc = read(fromfd, buffer, BUFFER_SIZE)) == -1) {
      error("read()");
    }
    if (!rc) {
      /* POLLIN @ EOF
    ...
    do {
      ...
      if (fds[1].revents & POLLOUT) {
        if ((c = write(tofd, buffer + wc, rc - wc)) == -1) {
          error("write()");
        }
        wc += c;
      }
